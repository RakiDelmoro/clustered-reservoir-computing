...
4. Update \`knowledge/index.md\` — IMPORTANT: you MUST preserve ALL existing entries from the Current Knowledge Index above. Only ADD new entries for new articles, UPDATE entries for modified articles, and keep all other existing entries unchanged. Do NOT drop any existing index entry.
...
Begin compilation now.`
  ...
  // Count existing index entries before writing (safety net against index wipe)
  const existingIndexEntries = (indexContent.match(/\[\[[^\]]+\]\]/g) || []).length
  ...
  // Safety net: verify index wasn't wiped (lost more than half its entries)
  const newIndexContent = await readText(absRoot, INDEX_FILE)
  const newIndexEntries = (newIndexContent.match(/\[\[[^\]]+\]\]/g) || []).length
  if (existingIndexEntries > 0 && newIndexEntries < Math.ceil(existingIndexEntries * 0.5)) {
    await debugLog(absRoot, `compileDailyLog WARNING: index entries dropped from ${existingIndexEntries} to ${newIndexEntries} — restoring original index`)
    await writeText(absRoot, INDEX_FILE, indexContent)
  }
